...
def get_train_test_data(train_esm2_p_path, train_esm2_n_path, train_ankh_p_path, train_ankh_n_path,
                        test_esm2_p_path, test_esm2_n_path, test_ankh_p_path, test_ankh_n_path):
    # 训练集 ESM-2
    with open(train_esm2_p_path, "rb") as tf:
        feature_dict = torch.load(tf, map_location=torch.device('cpu'))
        train_ESM2_P = feature_dict.float().numpy()
    with open(train_esm2_n_path, "rb") as tf:
        feature_dict = torch.load(tf, map_location=torch.device('cpu'))
        train_ESM2_N = feature_dict.float().numpy()
    train_ESM2 = np.vstack((train_ESM2_P, train_ESM2_N))

    # 训练集 Ankh
    with open(train_ankh_p_path, "rb") as f:
        feature_dict = torch.load(f, map_location=torch.device('cpu'))
        train_Ankh_P = feature_dict.numpy()
    with open(train_ankh_n_path, "rb") as f:
        feature_dict = torch.load(f, map_location=torch.device('cpu'))
    ...
    # 测试集 Ankh
    with open(test_ankh_p_path, "rb") as f:
        feature_dict = torch.load(f, map_location=torch.device('cpu'))
        test_Ankh_P = feature_dict.numpy()
    with open(test_ankh_n_path, "rb") as f:
        feature_dict = torch.load(f, map_location=torch.device('cpu'))
...
class Net(nn.Module):
    ...
    def forward(self, esm2_x, ankh_x, return_intermediate=False):
        esm2 = esm2_x
        ankh = ankh_x
        esm2_cnn_1 = self.esm2_cnn1(esm2).permute(0, 2, 1)
        ankh_cnn_1 = self.ankh_cnn1(ankh).permute(0, 2, 1)
        esm2_cnn_gru = self.esm2_cnn_gru(esm2).permute(0, 2, 1)
        ankh_cnn_gru = self.ankh_cnn_gru(ankh).permute(0, 2, 1)
        gru_inputs = torch.concat([esm2_cnn_gru, ankh_cnn_gru], dim=-1)
        gru_out, _ = self.bigru(gru_inputs)
        concat_out = torch.concat([esm2_cnn_1, gru_out, ankh_cnn_1], dim=-1)
        concat_out = concat_out.squeeze(1)
...
def train_five_fold_CV(X_train, y_train, model_dir):
    X_train_esm2, X_train_ankh = X_train
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X_train_esm2)):
        ...
        model, fold_mcc, model_history = train_model(
            (X_train_CV_esm2, X_train_CV_ankh), y_train_CV, (X_valid_CV_esm2, X_valid_CV_ankh), y_valid_CV
        )
        model.eval()
        with torch.no_grad():
        ...
        metrics = {
            'ACC': metric[0], 'PRE': metric[1], 'SN': metric[2], 'SP': metric[3],
            'F1': metric[4], 'AUC': metric[5], 'MCC': metric[6], 'AUPR': prc_data[2]
        }
        ...
        fold_model_path = os.path.join(model_dir, f'fold_{fold+1}_model.pt')
...
def evaluate_model(model, X_test, y_test, models=None, dataset_name="Test Set", output_path=None):
    ...
    with torch.no_grad():
        ...
        metrics = {
            'ACC': metric[0], 'PRE': metric[1], 'SN': metric[2], 'SP': metric[3],
            'F1': metric[4], 'AUC': metric[5], 'MCC': metric[6], 'AUPR': prc_data[2],
            'fpr': roc_data[0], 'tpr': roc_data[1], 'precision': prc_data[1], 'recall': prc_data[0]
        }
...
def main():
    parser = argparse.ArgumentParser(description='Run test or train.', add_help=False)
    parser.add_argument('-m', type=str, help='Path to model file (for testing) or save path (for training).')
    parser.add_argument('-i', type=str, required=True, help='Directory containing data files.')
    parser.add_argument('-o', type=str, required=True, help='Path to save predictions CSV.')
    parser.add_argument('-t', action='store_true', help='Run in training mode.')
    args = parser.parse_args()

    print("加载数据...")
    data_dir = args.i
    train_esm2_p_path = os.path.join(data_dir, 'train_esm2_p.pt')
    train_esm2_n_path = os.path.join(data_dir, 'train_esm2_n.pt')
    train_ankh_p_path = os.path.join(data_dir, 'train_ankh_p.pt')
    train_ankh_n_path = os.path.join(data_dir, 'train_ankh_n.pt')
    test_esm2_p_path = os.path.join(data_dir, 'test_esm2_p.pt')
    test_esm2_n_path = os.path.join(data_dir, 'test_esm2_n.pt')
    test_ankh_p_path = os.path.join(data_dir, 'test_ankh_p.pt')
    test_ankh_n_path = os.path.join(data_dir, 'test_ankh_n.pt')

    X_train, X_test, y_train, y_test, scaler_esm2, scaler_ankh, pca_esm2, pca_ankh = get_train_test_data(
        train_esm2_p_path, train_esm2_n_path, train_ankh_p_path, train_ankh_n_path,
        test_esm2_p_path, test_esm2_n_path, test_ankh_p_path, test_ankh_n_path
    )
    X_train_esm2, X_train_ankh = X_train
    X_test_esm2, X_test_ankh = X_test
    ...
    if args.t:
        print("\n运行训练模式...")
        model_dir = args.m if args.m else 'models'
        models, metrics_collection = train_five_fold_CV(X_train, y_train, model_dir)
        print("\n在测试集上进行评估（使用集成模型）...")
        evaluate_model(models[0], X_test, y_test, models=models, dataset_name="Test Set", output_path=args.o)
    else:
        if not args.m:
            raise ValueError("测试模式下必须提供模型路径 (-m)。")
        print(f"\n运行测试模式，加载模型: {args.m}")
        model = Net().to(device)
        model.load_state_dict(torch.load(args.m, map_location=device))
        evaluate_model(model, X_test, y_test, models=None, dataset_name="Test Set", output_path=args.o)
